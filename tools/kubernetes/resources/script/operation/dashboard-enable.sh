...
[[ $(hostname) != "cloudinfra-k8s-master" ]] && echo "Script must be run in the master node" && exit 1

# Hack: expliciting the node where to deploy (cloudinfra-k8s-master) in the kubernetes-dashboard Deployment
# and avoid the routing issues in other nodes
      ...
      containers:
        - name: kubernetes-dashboard
          ...
          securityContext:
            ...
            runAsUser: 1001
            runAsGroup: 2001
      nodeName: cloudinfra-k8s-master
      volumes:
        - name: kubernetes-dashboard-certs
          secret:
...
# Checking if Dashboard is accessible
...
## If so, connect to it - otherwise indicate the error (cluster may need to be recreated)
if [[ $? -eq 0 ]]; then
  ...
  echo "Dashboard located at http://127.0.0.1:8001/api/v1/namespaces/kubernetes-dashboard/services/https:kubernetes-dashboard:/proxy/"
  echo "- Login with the following token: ${TOKEN}"
    ...
    echo "Dashboard (${kube_dashboard_pod_ip}) cannot be reached. There may be an issue with the cluster networking"
fi
